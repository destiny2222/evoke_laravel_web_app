            ...
            <form action="{{  route('merchandise-payment') }}" method="post">
                ...
                <div id='form'>
                    <div class="slide-one">
                        ...
                        <br>
                        <p>Amount: ${{ number_format($merchandise->amount, 2) }} </p>
                        <p>Service Charge: {{ $charge->merchant_charge_amount }}%</p>
                        <p>Total amount: <span >${{ number_format( $merchandise->total_amount, 2)  }}</span></p>
                        <div>
                            <button type="button"  class="submit-form w-100 next">Confirm and Continue</button>
                        </div>
                    </div>
                    <input type="text" name="amount" value="{{ number_format($merchandise->amount, 2) }}" hidden style="border: none" id="">
                    <input type="text" name="total" class="total-amount" id="" hidden value="{{ number_format( $merchandise->total_amount, 2)  }}"  style="border:none">  
                    <div class="slide-two">
